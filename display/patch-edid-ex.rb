...
displays.each do |disp|
    ...
    # Assume one extension block
    puts "Extension block 1 byte 3: #{bytes[128 + 3]}"

    puts "Setting color support to RGB 4:4:4 only"
    bytes[128 + 3] &= ~(0b110000)
    bytes[128 + 127] = (0x100-(bytes[128..(128+126)].reduce(:+) % 256)) % 256
    puts "Recalculated extension checksum: 0x%x" % bytes[128 + 127]
